  ...
  return (
    <div data-testid="rescue-register-page" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28 grid md:grid-cols-12 gap-x-14 gap-y-10">
      <header className="md:col-span-5">
        <div className="font-mono text-[11px] uppercase tracking-[0.25em] text-[var(--gpr-muted)] mb-6">For Rescue Organisations</div>
        <h1 className="font-display font-extrabold text-5xl md:text-6xl text-[var(--gpr-primary)] leading-[0.98]">Register your <em className="accent text-[var(--gpr-alert)]">rescue</em>.</h1>
        <p className="font-serif italic text-[var(--gpr-muted)] text-lg mt-7 leading-[1.6]">Verified rescues manage cases, contact owners securely and log reunifications. Charity number speeds verification.</p>
      </header>
      <form onSubmit={submit} className="md:col-span-6 md:col-start-7 space-y-10">
        ...
        <div className="pt-6 border-t border-[var(--gpr-border)] flex justify-end">
          <button data-testid="rescue-submit" disabled={loading} className="gpr-btn-primary">{loading? "Submitting…" : "Submit for verification →"}</button>
        </div>
      </form>
    </div>
  );
